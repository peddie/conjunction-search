...
def load_stars(max_magnitude=5):
    ...
    with load.open(hipparcos.URL) as f:
        ...
        print('[  CATALOG] Found {} stars brighter than magnitude {}.'.format(len(df),
                                                                              max_magnitude))
class Observation:
    ...
    def summarize(self):
        ...
        return f"""{self.obj.name}, at {self.t} UTC:
  altitude {altaz[0]}
  azimuth  {altaz[1]}
  magnitude {magnitude}
...
class Site:
    ...
    def observe_solar_system(self, t, obj):
        obj.ephem_obj.compute(t)
        return Observation(t, self, obj, obj.ephem_obj.mag, obj.ephem_obj.radius)
    ...
    def next_set_rise(self, day):
        ...
        times = [t.astimezone(utc) for t in times]
...
class Star():
    def __init__(self, name, skyfield_obj):
        self.name = name
        self.skyfield_obj = skyfield_obj
...
class Constraint():
    def __init__(self, max_magnitude, max_distance):
...
class Conjunction():
    ...
    def __str__(self):
        return f"""Conjunction between {self.first_obs.obj.name} and {self.second_obs.obj.name}:
                time: {self.time}
            distance: {self.__distance() * 180 / np.pi} degrees
magnitude difference: {self.__magnitude_difference()} ({np.power(2.152, self.__magnitude_difference())} times brighter)
   maximum magnitude: {self.__max_magnitude()}
               score: {self.score()}
"""
...
def scan_night(site, day, constraint):
    ...
    conjunctions = {}
    for t in datetime_range(sunset, sunrise, dt):
        # Form all the possible conjunction pairs
        conj = [
            Conjunction(site.observe_solar_system(t, p), site.observe_solar_system(t, q), constraint)
            for p in solar_system
            for q in solar_system
            if p != q
        ]
    ...
    return sort_conjunctions_by_score(outputs)


def scan_days(location, t0_, t1_, constraint=Constraint(5.0, 1 * np.pi / 180)):
    t0 = t0_.replace(tzinfo=utc)
    t1 = t1_.replace(tzinfo=utc)

    dday = timedelta(days=1)

    best_conjunctions = {}

    for day in datetime_range(t0, t1, dday):
        best_conjunctions[day] = scan_night(location, day, constraint)
...
if __name__ == '__main__':
    ...
    brisbane = Site('27.49665 S', '152.9883 E', "Brisbane")

    t0 = datetime(2019, 9, 10).replace(tzinfo=utc)
    t1 = datetime(2019, 9, 25).replace(tzinfo=utc)

    results = scan_days(brisbane, t0, t1)
    display_results(results, brisbane, t0, t1)

    load_stars()
    print("Star conjunction support coming soon!")
